        ...
        <form className="mx-auto mt-8 flex flex-col gap-4" onSubmit={handleSubmit}>
            <div className="flex flex-col md:flex-row gap-1 mx-6 p-5 bg-[#eeeee4] justify-between rounded-2xl">
                <div className="flex flex-col gap-2 mx-auto">
                    <input className="bg-white px-4 w-75 rounded-lg border border-gray-700" type="text" value={title} onChange={(e) => setTitle(e.target.value)}placeholder="Enter title..."required/>
                    <input className="bg-white px-4 w-75 rounded-lg border border-gray-700" type="text" value={topicName} onChange={(e) => setTopicName(e.target.value)} placeholder="Enter topic name..." required/>
                </div>
                <div className="my-2 flex flex-col gap-2 md:flex-row mx-auto">
                    <select className="bg-[#8ad2c7] w-43.75 p-2.5 rounded-lg" value={category} onChange={(e) => setCategory(e.target.value)} required >
                        <option value="">Select Category</option>
                        <option value="Exam">Exam</option>
                        <option value="Event">Event</option>
                        <option value="General">General</option>
                    </select>
                    <select className="bg-[#8ad2c7] w-43.75 p-2.5" value={priority} onChange={(e) => setPriority(e.target.value)} required >
